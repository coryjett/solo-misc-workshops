...
| | **[Double OAuth Flow (Eager)](flows/flow-04b-double-oauth-eager-consent/)** | User-delegated double OAuth where the upstream leg runs **up front (eager)** via the gateway issuer — the MCP client completes the full OAuth handshake before the first tool call, so there's no `PENDING` URL and no separate approval UI |

...
| **Token Exchange** | **[Double OAuth Flow](flows/flow-04-double-oauth.md)** | OIDC auth + elicitation — two sequential user-facing OAuth flows for downstream auth and upstream API credentials |
...
- **[OBO Token Exchange](../obo-token-exchange-enablement/Agentgateway-OBO-Token-Exchange.md)** — How the built-in STS works, delegation vs impersonation, gateway-mediated vs agent-initiated exchange, `agentsts-adk` SDK integration, audience/scopes/claim generation, and downstream policy enforcement

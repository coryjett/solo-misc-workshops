...
| | **[Double OAuth — Eager + Consent](flows/flow-04b-double-oauth-eager-consent/)** | Same trust model as Flow 4, but upstream OAuth runs **up front (eager)** via the gateway issuer with an optional gateway-hosted consent screen — no `PENDING` URL, no separate UI |

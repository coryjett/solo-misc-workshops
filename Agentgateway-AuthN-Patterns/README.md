...
|---|---|---|
| **Inbound** | **[API Key Auth](flows/flow-08-api-key-auth/)** | Authenticate with static API key validated against K8s secrets |
...
| **Token Exchange** | **[Double OAuth Flow](flows/flow-04-double-oauth/)** | OIDC auth + elicitation — two sequential user-facing OAuth flows for downstream auth and upstream API credentials (**lazy**: upstream creds gathered on first tool call) |
| | **[Double OAuth — Eager + Consent](flows/flow-04b-double-oauth-eager-consent/)** | Same trust model as Flow 4, but upstream OAuth runs **up front** via the gateway issuer with an optional gateway-hosted consent screen — no `PENDING` URL, no separate UI |
...
| **Upstream Auth** | **[Claim-Based Token Mapping](flows/flow-07-claim-based-mapping/)** | Map JWT claims (sub, team) to per-user/group static tokens via CEL transformation |
| | **[Static Secret Injection](flows/flow-06-static-secret/)** | Validate inbound JWT, replace with a shared static credential from K8s secret |
| **Credential Gathering** | **[Elicitation](flows/flow-03-elicitation/)** | Out-of-band OAuth flow to collect upstream API credentials when they don't exist yet |

Plus a **[Decision Flowchart](flows/decision-flowchart.md)** to help choose the right pattern for a given scenario.

## Working Examples

Each flow includes a working example under its `example/` directory with a `setup.sh` that deploys everything from scratch (k3d cluster, AGW Enterprise, Keycloak if needed). Shared infrastructure scripts are in [`flows/common/`](flows/common/).

```bash
export AGENTGATEWAY_LICENSE_KEY="<your-license-key>"
cd flows/flow-08-api-key-auth/example
./setup.sh
```

#!/usr/bin/env bash
# Flow 2a: OBO Delegation (Dual Identity) — working example
# Agent exchanges user JWT + actor token at built-in STS for OBO token with sub + act.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
KEYCLOAK_REALM="flow02a-realm"
    ...
    class H(BaseHTTPRequestHandler):
        def do_POST(self):
            ...
            if method == 'initialize':
                resp = {"jsonrpc":"2.0","id":req_id,"result":{"protocolVersion":"2024-11-05","capabilities":{"tools":{"listChanged":False}},"serverInfo":{"name":"obo-test","version":"1.0"}}}
            elif method == 'notifications/initialized':
                self.send_response(200); self.end_headers(); return
            elif method == 'tools/list':
                resp = {"jsonrpc":"2.0","id":req_id,"result":{"tools":[{"name":"whoami","description":"Show token identity","inputSchema":{"type":"object","properties":{}}}]}}
            elif method == 'tools/call':
                result = {"token_issuer": claims.get("iss","none") if claims else "none", "sub": claims.get("sub","none") if claims else "none", "act": claims.get("act") if claims else None}
...
# ── Gateway + MCP Backend + Auth + Token Exchange policy ─────────────────────
info "Creating Gateway with MCP auth + OBO token exchange..."
kubectl apply -f - <<EOF
...
echo ""
echo "=== Testing Flow 2a: OBO Delegation ==="
echo ""
...
if [[ -z "$SID" ]]; then
  warn "Could not get MCP session (init response: ${INIT})"
else
  # Call whoami
  ...
  if echo "$RESULT" | jq -e '.token_issuer' >/dev/null 2>&1; then
    ISSUER=$(echo "$RESULT" | jq -r '.token_issuer')
    ACT=$(echo "$RESULT" | jq -r '.act // "null"')
    if [[ "$ISSUER" == *"7777"* ]] || [[ "$ISSUER" == *"agentgateway"* ]]; then
      ok "Token exchanged! MCP server received STS token (not Keycloak)"
    fi
    echo "$RESULT" | jq .
  else
...
echo ""
ok "Flow 2a: OBO Delegation — test complete"
echo "  Check MCP server logs: kubectl logs -l app=mcp-server --tail=10"

...
After running `setup.sh`, the gateway is port-forwarded to `localhost:8888`. The setup script automatically tests the flow by calling the `whoami` MCP tool and verifying the STS-issued token. You can also test manually:

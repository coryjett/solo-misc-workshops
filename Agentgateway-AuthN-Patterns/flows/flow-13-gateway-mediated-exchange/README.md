...
After running `setup.sh`, the gateway is port-forwarded to `localhost:8888`. The setup script automatically tests by calling the `echo_token` MCP tool and verifying the STS-issued token. You can also test manually:

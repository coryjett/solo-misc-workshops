    ...
    class H(BaseHTTPRequestHandler):
        def do_POST(self):
            ...
            if method == 'initialize':
                ...
                result = {"token_issuer": claims.get("iss","none") if claims else "no token received", "token_sub": claims.get("sub","none") if claims else "none", "token_act": claims.get("act") if claims else None, "message": "This is the token that reached the MCP server after elicitation"}

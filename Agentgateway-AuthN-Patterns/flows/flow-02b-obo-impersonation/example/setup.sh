#!/usr/bin/env bash
# Flow 2b: OBO Impersonation (Token Swap) — working example
# Same as 2a but without actor token — STS JWT has same sub, no act claim.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
KEYCLOAK_REALM="flow02b-realm"
  ...
  server.py: |
    ...
    class H(BaseHTTPRequestHandler):
        def do_POST(self):
            ...
            if claims:
                sys.stderr.write(f"\nMCP TOKEN: iss={claims.get('iss')} sub={claims.get('sub')} act={claims.get('act','NONE')}\n")
                sys.stderr.flush()
            ...
            if method == 'initialize':
                ...
                result = {"token_issuer": claims.get("iss","none") if claims else "none", "sub": claims.get("sub","none") if claims else "none", "act": claims.get("act") if claims else None, "has_act_claim": claims.get("act") is not None if claims else False}
...
# ── ReferenceGrant + Gateway + Policy (same structure as 2a) ─────────────────
kubectl apply -f - <<EOF
...
echo ""
echo "=== Testing Flow 2b: OBO Impersonation (no act claim) ==="
echo ""
...
if [[ -n "$SID" ]]; then
  ...
  echo "$RESULT" | jq .
  HAS_ACT=$(echo "$RESULT" | jq -r '.has_act_claim // false')
  ok "Token exchanged. has_act_claim=${HAS_ACT} (impersonation = no act claim)"
else
  warn "Could not get MCP session"
fi

echo ""
ok "Flow 2b: OBO Impersonation — test complete"

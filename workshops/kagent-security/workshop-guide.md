...
### Step 2 — Apply the EnterpriseAgentgatewayPolicy

The policy is pre-baked at `access-policy.yaml` (generated by `setup.sh` with the kagent OBO JWKS embedded). Apply it from a terminal alongside the demo:

```bash
kubectl apply -f access-policy.yaml
```

Show the policy contents to the audience:
...
Key fields:
- `targetRefs` → `agent-security-auditor-waypoint` Gateway (auto-provisioned by kagent because the Agent has the `kagent.solo.io/waypoint: "true"` label)
- `jwtAuthentication.providers[0].issuer: kagent.kagent` — validates the OBO token kagent mints for each user
- `authorization.policy.matchExpressions: 'jwt.Groups.exists(g, g == "admins")'` — CEL expression on the OIDC `Groups` claim propagated into the OBO token
...
> **Optional UI view:** Navigate to **Access Policies** in the kagent UI to show the auto-generated `AccessPolicy` resource view.

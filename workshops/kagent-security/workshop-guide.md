...
> **Skip the AGE tab.** The Solo Enterprise UI has two products in the launcher (top-left). This demo lives entirely in the **kagent** product. The **agentgateway (AGE)** dashboards are empty by design — our agents call OpenAI directly, not through an AGW Gateway, so there's nothing for the AGE tracing/metrics views to show.

> **Quick "what's running" tour** (optional, before the demo):
> ```bash
> ./status.sh
> ```
> One-screen view of the cluster, agent pods, waypoint, AccessPolicy state, and the JWT `role` claim users will get.
...
> **If admin ALSO gets 403** during this step, the user's browser has a Keycloak token issued before the `role` mapper was wired in. Quick recovery:
> 1. Sign Out in the kagent UI (top-right user menu)
> 2. Hit `http://<your-ip>:8088/realms/kagent-dev/protocol/openid-connect/logout` once to clear the SSO cookie
> 3. Close the incognito window, open a new one, log in as admin again
>
> Or run `./whoami.sh admin` from a terminal to confirm the access token actually carries `role: 'admin'`.
